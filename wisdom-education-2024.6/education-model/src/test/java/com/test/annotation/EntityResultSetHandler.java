 ...
 * @since 1.0.0
 */
public class EntityResultSetHandler extends DefaultResultSetHandler {

    protected MappedStatement mappedStatement;
    protected ObjectFactory objectFactory;
    private static final Logger logger = LoggerFactory.getLogger(EntityResultSetHandler.class);

    public EntityResultSetHandler(Executor executor, MappedStatement mappedStatement,
    ...
    public List<Object> handleResultSets(Statement stmt) throws SQLException {
        ...
        if (ObjectUtils.isNotEmpty(resultMapList)) {
            for (ResultMap resultMap : resultMapList) {
                ...
                handlerValue((Class<? extends BaseEntity>) clazz, resultSet, multipleResults);
            }
        }
        return multipleResults;
    }

    private void handlerValue(Class<? extends BaseEntity> clazz, ResultSet resultSet, List<Object> multipleResults) {

        try {
            BaseEntity baseEntity = objectFactory.create(clazz);
            ...
            while (resultSet.next()) {
                ...
                multipleResults.add(baseEntity);
            }
        } catch (Exception e) {
